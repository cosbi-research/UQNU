...
debug_folder = "debug_lorenz"
result_folder = "result_lorenz"
...
ensemble_number = starting_point_index
...
include("configurations_lorenz.jl")

################################### loads the data ##############################################
training_data_structure = deserialize("./data_generator/lorenz_training_data_structure_err_1.jld")
bounding_box = deserialize("./data_generator/lorenz_in_silico_data_bounding_box.jld")
#generate a grid 100*100
xrange_bounding_box = range(bounding_box[1], bounding_box[2], length=5)
yrange_bounding_box = range(bounding_box[3], bounding_box[4], length=5)
zrange_bounding_box = range(bounding_box[5], bounding_box[6], length=5)

################################### loads the single-result #####################################
#load the result of the single-parameter training
trained_ensemble = deserialize("training_UDE_results/lorenz/ensemble_results_model_2_with_seed.jld")

#sort them according to status and validation likelihood
trained_ensemble = [res for res in trained_ensemble if res.status == "success" && res.validation_likelihood < 0.01]
...
original_σ = parameters.σ * (upper_bounds[1] - lower_bounds[1]) + lower_bounds[1]
original_r = parameters.r * (upper_bounds[2] - lower_bounds[2]) + lower_bounds[2]

parameters.σ = 1.0
parameters.r = 1.0

original_parameters_ude = [original_σ, original_r]
...
include("out_of_domain_variability_3d.jl")
using .out_of_domain_variability_3d
...
ood_analyzer = out_of_domain_variability_3d.out_of_domain_var_3d(xrange_bounding_box, yrange_bounding_box, zrange_bounding_box, vector_field_function, lorenz_ground_truth, experimental_points, [], [], [], [])
out_of_domain_variability_3d.computeGroundTruth(ood_analyzer)
out_of_domain_variability_3d.computePoints(ood_analyzer)
...
function costFunctionOnSingleTraj(par, i, integrator, sensealg, prob_uode_tmp)
  ...
  cost_trajectory = 1 / size(original_solutions, 1) * (sum((simulation[1, :] - original_solutions.x1) .^ 2 ./ training_data_structure.max_oscillations[i][1]^2) + sum((simulation[2, :] - original_solutions.x2) .^ 2 ./ training_data_structure.max_oscillations[i][2]^2) + sum((simulation[3, :] - original_solutions.x3) .^ 2 ./ training_data_structure.max_oscillations[i][3]^2))
...
function getReoptimizedParameters(par, integrator, sensealg)
  ...
  original_parameters_ude_fixed = [original_parameters_ude[1] * par.σ, original_parameters_ude[2] * par.r] 
...
function flatten(min_x, min_y, min_z, max_x, max_y, max_z, x_size, y_size)
  flattened_positions = []

  for x in min_x:max_x
    for y in min_y:max_y
      for z in min_z:max_z
        # Convert (x, y, z) to 1D index assuming column-major order (like Julia uses)
        index = (z - 1) * x_size * y_size + (y - 1) * x_size + x
...
function getDistanceGradient(par, trajectories)
  #select just the first 10 points of the already done trajectiores
  tmp_trajectories = []
  for traj in trajectories 
    tmp_trajectory = []
    for i in 5:min(6, size(traj, 1))
      push!(tmp_trajectory, traj[i])
    end
    push!(tmp_trajectories, tmp_trajectory)
  end
  ...
  for traj in tmp_trajectories
    for point in traj 
      ...
      if isnothing(gradient)
        gradient = 2 * (par .- point)
      else
        gradient .= gradient .+ 2 * (par .- point)
      end
    end
  end
  
  gradient = gradient ./ number_of_points

  gradient[Not((end-1:end)), :] .= 0.0
...
function getVarianceGradient(par, points, total_ensemble, ood_analyzer)
  ...
  selected_x_index = sample(1:5, 1)[1]
  selected_y_index = sample(1:5, 1)[1]
  selected_z_index = sample(1:5, 1)[1]
 
 
  min_x_square = max(selected_x_index - 1, 1)
  min_y_square = max(selected_y_index - 1, 1)
  min_z_square = max(selected_z_index - 1, 1)
  max_x_square = min(selected_x_index + 1, 5)
  max_y_square = min(selected_y_index + 1, 5)
  max_z_square = min(selected_z_index + 1, 5)
 
  selected_indexes = flatten(min_x_square, min_y_square, min_z_square, max_x_square, max_y_square, max_z_square, 5, 5)

  selected_tmp_points = points[:, selected_indexes] 

  ensemble_predictions = out_of_domain_variability_3d.get_ensemble_predictions(ood_analyzer, total_ensemble, selected_tmp_points)
  current_prediction = out_of_domain_variability_3d.get_ensemble_predictions(ood_analyzer, [par], selected_tmp_points)[1]
  ...
  variance_y1 = 1 / size(total_ensemble, 1) .* abs2.(current_prediction[1, :] .- mean_prediction[1, :])
  ...
  variance_y2 = 1 / size(total_ensemble, 1) .* abs2.(current_prediction[2, :] .- mean_prediction[2, :])

  gradient_in_single_points_y2 = gradient_in_single_points_y2 ./ variance_y2

  #Third variable of the ensemble
  vector_field = x -> vector_field_function(selected_tmp_points, x)[3, :]
  gradient_vector_field = jacobian(x -> vector_field(x), par)[1]

  #rows: gradient of the vector field (third variable) in a point
  gradient_in_single_points_y3 = 2 / size(total_ensemble, 1) .* (current_prediction[3, :] .- mean_prediction[3, :]) .* gradient_vector_field
  variance_y3 = 1 / size(total_ensemble, 1) .* abs2.(current_prediction[3, :] .- mean_prediction[3, :])

  gradient_in_single_points_y3 = gradient_in_single_points_y3 ./ variance_y3

  #@infiltrate

  #gradient_in_single_points_y1[Not(selected_indexes), 1:end] .= 0.0
  #gradient_in_single_points_y2[Not(selected_indexes), 1:end] .= 0.0
  #gradient_in_single_points_y3[Not(selected_indexes), 1:end] .= 0.0

  gradient_variance = mean(gradient_in_single_points_y1, dims=1) .+ mean(gradient_in_single_points_y2, dims=1) .+ mean(gradient_in_single_points_y3, dims=1)
...
function getCovarianceGradient(par, points, total_ensemble, ood_analyzer)
  ...
  #Third variable of the ensemble
  vector_field = x -> vector_field_function(selected_tmp_points, x)[3, :]
  gradient_vector_field_y3 = jacobian(x -> vector_field(x), par)[1]

  #rows: gradient of the vector field (third variable) in a point
  gradient_in_single_points_variance_y3 = 2 * (size(total_ensemble, 1)-1) / size(total_ensemble, 1)^2 .* (current_prediction[3, :] .- mean_prediction[3, :]) .* gradient_vector_field_y3
  variance_y3 = zeros(size(current_prediction[3, :]))
  for i in axes(stacked_predictions, 3)
    variance_y3 .+= 1 / size(total_ensemble, 1) .* abs2.(stacked_predictions[3, :, i] .- mean_prediction[3, :])
  end

  gradient_in_single_points_variance_y3 = gradient_in_single_points_variance_y3 
  ...
  covariance_y1_y3 = zeros(size(current_prediction[1, :]))
  for i in axes(stacked_predictions, 3)
    covariance_y1_y3 .+= 1 / size(total_ensemble, 1) .* (stacked_predictions[1, :, i] .- mean_prediction[1, :]) .* (stacked_predictions[3, :, i] .- mean_prediction[3, :])
  end

  covariance_y2_y3 = zeros(size(current_prediction[1, :]))
  for i in axes(stacked_predictions, 3)
    covariance_y2_y3 .+= 1 / size(total_ensemble, 1) .* (stacked_predictions[2, :, i] .- mean_prediction[2, :]) .* (stacked_predictions[3, :, i] .- mean_prediction[3, :])
  end
  ...
  gradient_cov_y1_y3 = (size(total_ensemble, 1)-1) / size(total_ensemble, 1)^2 .* (gradient_vector_field_y1 .* (current_prediction[3, :] .- mean_prediction[3, :]) .+ gradient_vector_field_y3 .* (current_prediction[1, :] .- mean_prediction[1, :])) 
  gradient_cov_y2_y3 = (size(total_ensemble, 1)-1) / size(total_ensemble, 1)^2 .* (gradient_vector_field_y3 .* (current_prediction[3, :] .- mean_prediction[3, :]) .+ gradient_vector_field_y3 .* (current_prediction[2, :] .- mean_prediction[2, :])) 
  
  gradient_var = gradient_in_single_points_variance_y1 .* variance_y2 .* variance_y3 .+ gradient_in_single_points_variance_y2 .* variance_y1 .* variance_y3 .+ gradient_in_single_points_variance_y3 .* variance_y1 .* variance_y2
  gradient_cov_term = 2 * gradient_cov_y1_y2 .* covariance_y1_y3 .* covariance_y2_y3 .+ 2 * gradient_cov_y1_y3 .* covariance_y1_y2 .* covariance_y2_y3 .+ 2 * gradient_cov_y2_y3 .* covariance_y1_y2 .* covariance_y1_y3
  gradient_cov_negative_term = - gradient_in_single_points_variance_y1 .* (covariance_y2_y3.^2) - 2 .* covariance_y2_y3 .* gradient_cov_y2_y3 .* variance_y1 .- gradient_in_single_points_variance_y2 .* (covariance_y1_y3.^2) - 2 .* covariance_y1_y3 .* gradient_cov_y1_y3 .* variance_y2 .- gradient_in_single_points_variance_y3 .* (covariance_y1_y2.^2) - 2 .* covariance_y1_y2 .* gradient_cov_y1_y2 .* variance_y3

  total_gradient = gradient_var .+ gradient_cov_term .+ gradient_cov_negative_term

  determinant = variance_y1 .* variance_y2 .* variance_y3 + 2 .* covariance_y1_y2 .* covariance_y1_y3 .* covariance_y2_y3 - abs2.(covariance_y2_y3) .* variance_y1 - abs2.(covariance_y1_y3) .* variance_y2 - abs2.(covariance_y1_y2) .* variance_y3
...
function getNextPointDirection(par, times, initial_states, training_data_structure, out_of_domain_points, current_ensemble, ood_analyzer, parameter_index, step_size, trajectories, iteration)
...
function getValidationCost(pars, initial_states, integrator=integrator, sensealg=sensealg)
  cost = 0.0
  ...
  for i in 1:3
    ...
    simulation = model_simulation(pars, tmp_times, i, initial_states, integrator, sensealg)
    ...
    cost_trajectory = 1 / size(training_data_structure.solution_dataframes[i], 1) * (sum((simulation[1, :] - training_data_structure.solution_dataframes[i].x1) .^ 2 ./ training_data_structure.max_oscillations[i][1]^2) + sum((simulation[2, :] - training_data_structure.solution_dataframes[i].x2) .^ 2 ./ training_data_structure.max_oscillations[i][2]^2) + sum((simulation[3, :] - training_data_structure.solution_dataframes[i].x3) .^ 2 ./ training_data_structure.max_oscillations[i][3]^2))
...
#number_iterations_for_trajectory = 1500
...
#trajectories = previous_trajectories[1:2]
trajectories = []
...
initial_cost = getValidationCost(parameters, initial_states, integrator, sensealg)

if initial_cost > validation_cost_threshold
  ensemble_interval_begin = (ensemble_number - 1) * 5 + 1
  ensemble_interval_end = (ensemble_number - 1) * 5 + 1 + 4
  ...
  for new_index in remaining_indexes
    ...
    global initial_cost = getValidationCost(parameters, initial_states, integrator, sensealg)
...
if initial_cost > validation_cost_threshold
  ...
  for traj_number in 1:number_of_trajectories
    ...
    try
      ...
      global integrator = Vern7()
      global sensealg = InterpolatingAdjoint(autojacvec=ReverseDiffVJP(true))
      ...
      while iteration_performed < 200
        ...
        for iterator in 1:number_iterations_for_trajectory
          ...
          if iterator == 1
            ...
            new_suggestion_direction = getNextPointDirection(current_ensemble[end], tmp_times, initial_states, training_data_structure, selected_points, current_ensemble, ood_analyzer, sampled_index_for_trajectory, step_size, trajectories, iterator)
            ...
            validation_cost = Inf
            try
              validation_cost = getValidationCost(new_proposed_parameters, initial_states, integrator, sensealg)
            catch
          ...
          if validation_cost > validation_cost_threshold
            ...
            try
              ...
              validation_cost = getValidationCost(new_suggestion, initial_states, integrator, sensealg)
          ...
          if length(current_ensemble) == 1
            ...
            average_variance_before = out_of_domain_variability_3d.getVarianceInPoints(ood_analyzer, current_ensemble, selected_points)
          end
          ...
          variance = out_of_domain_variability_3d.getVarianceInPoints(ood_analyzer, current_ensemble, selected_points)
          ...
          @info "Physical parameters values: " * string(new_suggestion.σ * original_σ) * ", " * string(new_suggestion.r * original_r) * ", original values " * string(original_parameters.σ * original_σ) * ", " * string(original_parameters.r * original_r) 
          ...
          if size(current_ensemble, 1) > 3 && iterator % 10 == 0

            if loglevel <= Logging.Info
              out_of_domain_analysis = out_of_domain_variability_3d.getOutOfDomainAnalysis(ood_analyzer, current_ensemble)

              push!(cicps, out_of_domain_analysis.cicp)
              push!(areas, out_of_domain_analysis.mean_area)

              @info "******************************************************"
              @info "CICP: " * string(out_of_domain_analysis.cicp)
              @info "Mean area: " * string(out_of_domain_analysis.mean_area)

              #= @info "Visualizing the OOD statistics "
              out_of_domain_plots = out_of_domain_variability_3d.plotOutOfDomainAnalysis(ood_analyzer, out_of_domain_analysis)

              cicp_file_name = debug_folder * "/cicp_" * string(traj_number) * "_" * string(iterator) * ".png"
              savefig(out_of_domain_plots.cicp_out_of_domain, cicp_file_name)

              ellipse_file_name = debug_folder * "/ellipse_" * string(traj_number) * "_" * string(iterator) * ".png"
              savefig(out_of_domain_plots.areas_out_of_domain, ellipse_file_name)
              =#
              @info "Visualizing the training set trajectories"
              validation_traj_1_file_name = debug_folder * "/validation_traj_1_iter_"  * string(traj_number) * "_" * string(iterator) * ".png"
              traj_training_plot = diagnostic_training_set.printValidationPlots3d(new_suggestion, original_times, 1, initial_states, prob_uode_pred, integrator, reltol, abstol, training_data_structure)
              Plots.savefig(traj_training_plot, validation_traj_1_file_name)

              validation_traj_2_file_name = debug_folder * "/validation_traj_2_iter_" * string(traj_number) * "_" * string(iterator) * ".png"
              traj_training_plot = diagnostic_training_set.printValidationPlots3d(new_suggestion, original_times, 2, initial_states, prob_uode_pred, integrator, reltol, abstol, training_data_structure)
              Plots.savefig(traj_training_plot, validation_traj_2_file_name)

              validation_traj_3_file_name = debug_folder * "/validation_traj_3_iter_"  * string(traj_number) * "_" * string(iterator) * ".png"
              traj_training_plot = diagnostic_training_set.printValidationPlots3d(new_suggestion, original_times, 3, initial_states, prob_uode_pred, integrator, reltol, abstol, training_data_structure)
...
+      end
  ...
  #saving the ensemble with the actual parameters
  for i in axes(ensemble, 1)
    ensemble[i].σ = ensemble[i].σ * original_σ
    ensemble[i].r = ensemble[i].r * original_r
  end

  for i in axes(new_ensemble, 1)
    new_ensemble[i].σ = new_ensemble[i].σ * original_σ
    new_ensemble[i].r = new_ensemble[i].r * original_r
  end

  for i in axes(naive_ensemble_reference, 1)
    naive_ensemble_reference[i].σ = naive_ensemble_reference[i].σ * (upper_bounds[1] - lower_bounds[1]) + lower_bounds[1]
    naive_ensemble_reference[i].r = naive_ensemble_reference[i].r * (upper_bounds[2] - lower_bounds[2]) + lower_bounds[2]
  end
  ...
  #save the results
  serialize(result_folder * "/results.jld", results)
  serialize(result_folder * "/trajectories_lorenz.jld", trajectories)
  serialize(result_folder * "/variances_lorenz.jld", variances)
  serialize(result_folder * "/cicps_lorenz.jld", total_cicps)
end