...
function lotka_volterra_gound_truth(du, u, pars, t)
    α, β, γ, δ = original_parameters .* pars
    du[1] = α * u[1] - β * u[2] * u[1]
    du[2] = γ * u[1] * u[2] - δ * u[2]
end

tspan = extrema(training_data_structure.solution_dataframes[1].t)
prob_uode_pred = ODEProblem{true}(lotka_volterra_gound_truth, Array(training_data_structure.solution_dataframes[1][1, 2:(end-1)]), tspan)
...
eigenDecomposition = getEigenDempositionHessianNotProportional([1.0, 1.0, 1.0, 1.0], times, training_data_structure)
