...
projections_σ_null_space = []
projections_r_null_space = []
...
for ensemble in ensembles
    for model in ensemble
        ...
        projection_σ_null_space = nothing
        projection_r_null_space = nothing
        for i in 1:size(null_space_eigenvectors, 2)

            eigenvector = null_space_eigenvectors[:, i]


            tmp_σ = eigenvector[end-1] .* eigenvector
            tmp_r = eigenvector[end] .* eigenvector

            if isnothing(projection_σ_null_space)
                projection_σ_null_space = tmp_σ
                projection_r_null_space = tmp_r
            else
                projection_σ_null_space .+= tmp_σ
                projection_r_null_space .+= tmp_r
            end
        ...
        projection_σ_null_space = norm(projection_σ_null_space)
        projection_r_null_space = norm(projection_r_null_space)

        push!(projections_σ_null_space, projection_σ_null_space)
        push!(projections_r_null_space, projection_r_null_space)
    end
end

#plot the histogram of the projections_α_null_space
StatsPlots.histogram(projections_σ_null_space, bins=0:binwidth:1, title="", xlabel="Projection on the null space of σ", ylabel="Frequency", legend=false, dpi=400)
#VLINE DASHED RED FOR THE THRESHOLD 0.05
xlims!(0, 1)
vline!([0.05], color=:red, linestyle=:dash, label="", linewidth=2)
Plots.savefig("histogram_projections_σ_null_space.svg")

#plot the histogram of the projections_δ_null_space
StatsPlots.histogram(projections_r_null_space, bins=0:binwidth:1, title="", xlabel="Projection on the null space of r", ylabel="Frequency", legend=false, dpi=400)
#VLINE DASHED RED FOR THE THRESHOLD 0.05
xlims!(0, 1)
vline!([0.05], color=:red, linestyle=:dash, label="", linewidth=2)
Plots.savefig("histogram_projections_r_null_space.svg")