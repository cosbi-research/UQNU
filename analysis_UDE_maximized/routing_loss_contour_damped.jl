...
parameters.α = 1.0
original_parameters_ude = [original_α]
...
function getReoptimizedParameters(par, integrator=integrator, sensealg=sensealg)
  ...
  original_parameters_ude_fixed = [original_parameters_ude[1] * par.α]
...
total_parameter_indexes = 1:(length(parameters)-1)
...
sampled_indexes_trajectories = [vcat(sampled_indexes_trajectories[i], length(parameters)) for i in 1:number_of_trajectories]
    ...
    try
      ...
      while iteration_performed < 300
        ...
        for iterator in 1:number_iterations_for_trajectory
          ...
          @info "Physical parameters values: " * string(new_suggestion.α) * ", original values " * string(original_parameters.α)
