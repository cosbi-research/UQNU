...
function analyze_method(ensemble, nn_dimension, threshold)
...
seed = 0
rng = StableRNG(seed)
#sampled_seeds = sampled_seeds[1:20]

results_ensembles = [analyze_method(ensemble, 32, threshold) for ensemble in ensembles] 
...
plt = Plots.scatter([p[1] for p in points_to_plot], [p[2] for p in points_to_plot], label="", xlabel="x", ylabel="y", zcolor=ground_truth_in_ensemble_to_plot, color=:viridis, markerstrokewidth=0, markersize=4, dpi=400)
...
Plots.savefig(plt, "ground_truth_in_ensemble_extended.png")
Plots.savefig(plt, "ground_truth_in_ensemble_extended.svg")
