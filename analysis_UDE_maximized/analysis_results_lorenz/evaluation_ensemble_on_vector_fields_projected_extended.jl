    ...
    get_uode_model_function = function (appr_neural_network, state)
        ...
        f(u, p) =
            let appr_neural_network = appr_neural_network, st = state
                ...
                û = appr_neural_network(u_reprojected, p.p_net, st)[1]
                du = similar(u_reprojected)
                du[1] = p.σ*(u_reprojected[2] - u_reprojected[1]) 
                du[2] = u_reprojected[1]*(p.r-u_reprojected[3])- u_reprojected[2]
                du[3] = û[1]
                return du
            end
    end
...
mean_cicps = deserialize("cicps_distribution_extended.jld")
mean_cicp = mean(mean_cicps)

gr()
plt = Plots.plot(dpi=600)
...
Plots.plot!(plt, title="MOD (mean CP: $mean_cicp)", legend=:topleft)
Plots.plot!(plt, 
...
Plots.savefig(plt, "ground_truth_in_ensemble_projected_extended.svg")
Plots.savefig(plt, "ground_truth_in_ensemble_projected_extended.png")
