...
results_naive_deserialized = deserialize("results_analysis_naive.jld")
...
plt = Plots.scatter([p[1] for p in points_to_plot], [p[2] for p in points_to_plot], label="", xlabel="x", ylabel="y", zcolor=log10.(mean_areas_in_maximized_ensemble), color=:viridis, markerstrokewidth=0, markersize=4)
...
plt = Plots.scatter([p[1] for p in points_to_plot], [p[2] for p in points_to_plot], label="", xlabel="x", ylabel="y", zcolor=log10.(mean_areas_in_naive), color=:viridis, markerstrokewidth=0, markersize=4)
Plots.scatter!(plt, experimental_data[:, :x1], experimental_data[:, :x2], label="training data", legend=false)
Plots.plot!(plt, xlims=(min_x, max_x), ylims=(min_y, max_y))
