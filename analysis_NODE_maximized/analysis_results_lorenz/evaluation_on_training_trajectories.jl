...
# read the geometry of the vector field included in the bounding box
ground_truth_in_ensemble_grid = deserialize("ground_truth_in_ensemble_grid.jld")
ground_truth_in_ensemble_grid = ground_truth_in_ensemble_grid[1]
...
for i in trajectories
    ...
    for j in axes(ensembles, 1)
        ...
        initial_point = [initial_point.x1, initial_point.x2, initial_point.x3]
        ...
        for simulation in esemble_simulations
            ...
            Plots.plot!(p3_plot, simulation.t, simulation.x3, label="z", color="blue", leged=false)
        end
        ...
        Plots.scatter!(p3_plot, experimental_data_for_trajectory.t, experimental_data_for_trajectory.x3, label="", color="red", markersize=5, legend=false, xlabel="t", ylabel="z")
    end
    ...
    plt = Plots.plot(p1_plot, p2_plot, p3_plot, layout=(1, 3), size=(1800, 400))
    Plots.savefig(plt, joinpath(training_trajectories_folder, "ensemble_training_trajectories_$i.png"))
    Plots.savefig(plt, joinpath(training_trajectories_folder, "ensemble_training_trajectories_$i.svg"))
end
