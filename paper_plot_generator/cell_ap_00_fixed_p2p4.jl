...
fixing p2 and p4 to their literature values
=#
...
prob_uode_pred = ODEProblem{true}(uode_derivative_function, par_opt.initial_state, (0, maximum(solution_dataframe.t)))
solutions = solve(prob_uode_pred, TRBDF2(autodiff=false), p=parameters_optimized, saveat=saveat=0.001, abstol=abstol, reltol=reltol)

#ground truth dynamics
prob_ground_truth = ODEProblem{true}(ground_truth_function, original_u0, (0, maximum(solution_dataframe.t)))
simulation_data_ground_truth = solve(prob_ground_truth, TRBDF2(autodiff=false), p=original_parameters,  saveat=0.001, abstol=abstol)
...
savefig(plt, "plots/cell_ap_plot_fixed_"*error_level*"_summary.svg")
...
for i in 1:size(solution_dataframe)[2]-1
    ...
    #savefig(plt, "plots/cell_ap_plot_fixed_"*error_level*"_s"*string(i)*".pdf")
...
savefig(plt_y1y2, "plots/cell_ap_plot_fixed_"*error_level*"_y1y2.svg")