#=
Script to generate the plots for the cell apoptosis UDE model trained on a dataset with error level 5%
fixing p2 and p4 to their literature values, assuming that only the vairables 5 and 6 are observable
=#

cd(@__DIR__)

using ComponentArrays, Lux, Serialization, DifferentialEquations, Random, DataFrames, Plots
using StatsPlots, LaTeXStrings, Plots.PlotMeasures, DiffEqFlux, .Flux
...
par_opt = deserialize("local_optima_found/cell_ap_opt_05_fixed_p2p4_observable_56.jld")
...
uode_derivative_function = get_uode_model_function_fixed_p2p4(approximating_neural_network, par_opt.net_status, deepcopy(parameters_optimized.ode_par))
...
savefig(plt, "plots/cell_ap_plot_fixed_partially_obs_"*error_level*"_summary.svg")
...
plt_y5y6 = Plots.plot(plts[5], plts[6], layout=(1, 2), size=(1000, 300), legend=false, bottom_margin = 10mm, left_margin = 20mm, dpi = 300)

savefig(plt_y5y6, "plots/cell_ap_plot_partially_observable_"*error_level*"_y5y6.svg")
