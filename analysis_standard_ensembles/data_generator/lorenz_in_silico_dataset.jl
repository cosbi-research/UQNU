...
df = DataFrame(t = times, x1 = sol_as_array_1[1,:], x2 = sol_as_array_1[2,:], x3 = sol_as_array_1[3,:], traj = repeat([1], length(times)))
df = vcat(df, DataFrame(t = times, x1 = sol_as_array_2[1,:], x2 = sol_as_array_2[2,:], x3 = sol_as_array_2[3,:], traj = repeat([2], length(times))))
df = vcat(df, DataFrame(t = times, x1 = sol_as_array_3[1,:], x2 = sol_as_array_3[2,:], x3 = sol_as_array_3[3,:], traj = repeat([3], length(times))))
#save the data
serialize("lorenz_in_silico_data_no_noise_pure_trajectory.jld", df)
