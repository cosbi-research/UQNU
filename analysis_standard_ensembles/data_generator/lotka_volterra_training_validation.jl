...
for error_level in error_levels
    ...
    datafile = "lotka_volterra_in_silico_data"
    if error_level == 1
    ...
    pure_trajectory_file = "lotka_volterra_in_silico_data_no_noise_pure_trajectory.jld"
    ...
    plts_traj = []
    plt_extended_bounding_box = Plots.plot(legend=false)
    for traj in 1:3 
        ...
        Plots.scatter!(plt_extended_bounding_box, tmp_solution_dataframe[1:end, :x1], tmp_solution_dataframe[1:end, :x2], label="", color="blue")
        Plots.plot!(plt_extended_bounding_box, tmp_trajectory_dataframe[1:end, :x1], tmp_trajectory_dataframe[1:end, :x2], label="", color="blue", linewidth=0.5)
        ...
        #draw a rectangle on the bounding box of the training data in plts_extended_bounding_box
        min_x = minimum(tmp_training_dataframe.x1)
        max_x = maximum(tmp_training_dataframe.x1)
        min_y = minimum(tmp_training_dataframe.x2)
        max_y = maximum(tmp_training_dataframe.x2)
    ...
    min_x = minimum([minimum(solution_dataframes[i].x1) for i in 1:3])
    max_x = maximum([maximum(solution_dataframes[i].x1) for i in 1:3])
    min_y = minimum([minimum(solution_dataframes[i].x2) for i in 1:3])
    max_y = maximum([maximum(solution_dataframes[i].x2) for i in 1:3])

    #enlarge simmetrically by 20%
    min_x = min_x - 0.2 * (max_x - min_x)
    max_x = max_x + 0.2 * (max_x - min_x)
    min_y = min_y - 0.2 * (max_y - min_y)
    ...
    Plots.plot!(plt_extended_bounding_box, [min_x, max_x, max_x, min_x, min_x], [min_y, min_y, max_y, max_y, min_y], color=:black, fillalpha=0.0, fillcolor=:black, seriestype=:shape, label="")
    Plots.plot!(plt_extended_bounding_box, xlabel="x", ylabel="y", xlims=(min_plot_x, max_plot_x), ylims=(min_plot_y, max_plot_y))

    plt_data_structure = Plots.plot(plts_traj..., layout=(3, 1))

    Plots.savefig("lotka_volterra_training_data_structure_err_"*string(error_level)*".png")
    Plots.savefig(plt_extended_bounding_box, "lotka_volterra_training_data_structure_err_"*string(error_level)*"_extended_bounding_box.png")

    #save as png
    Plots.savefig("lotka_volterra_training_data_structure_err_"*string(error_level)*".png")

    serialize("lotka_volterra_training_data_structure_err_"*string(error_level)*".jld", data_structure)
end