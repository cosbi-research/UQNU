...
for error_level in error_levels
    ...
    datafile = "lorenz_in_silico_data"
    if error_level == 1
    ...
    pure_trajectory_file = "lorenz_in_silico_data_no_noise_pure_trajectory.jld"
    pure_trajectory_dataframe = deserialize(pure_trajectory_file)
    ...
    plt_extended_bounding_box = Plots.plot3d(legend=false)
    ...
    for traj in 1:3
        ...
        plt_v3 = Plots.plot(legend=false)

        tmp_training_dataframe = training_dataframes[traj]
        tmp_validation_dataframe = validation_dataframes[traj]
        tmp_solution_dataframe = solution_dataframes[traj]

        tmp_trajectory_dataframe = pure_trajectory_dataframe[pure_trajectory_dataframe.traj.==traj, :]
        ...
        Plots.scatter!(plt_v3, tmp_training_dataframe[1:end, :t], tmp_training_dataframe[1:end, :x3], label="x3 training", color="blue")
        Plots.scatter!(plt_v3, tmp_validation_dataframe[1:end, :t], tmp_validation_dataframe[1:end, :x3], label="x3 validation", color="red")
        #plot a vertical line for the max oscillation

        Plots.scatter!(plt_extended_bounding_box, tmp_solution_dataframe[1:end, :x1], tmp_solution_dataframe[1:end, :x2], tmp_solution_dataframe[1:end, :x3], label="", color="blue")
        Plots.plot!(plt_extended_bounding_box, tmp_trajectory_dataframe[1:end, :x1], tmp_trajectory_dataframe[1:end, :x2], tmp_trajectory_dataframe[1:end, :x3], label="", color="blue", linewidth=0.5)
        ...
        min_y = minimum(tmp_training_dataframe.x3)
        height = max_oscillations[traj][3]
        Plots.plot!(plt_v3, [x_min, x_max, x_max, x_min, x_min], [min_y, min_y, min_y + height, min_y + height, min_y], fillalpha=0.2, fillcolor=:green, label="")

        #put the plots together
        plt = Plots.plot(plt_v1, plt_v2, plt_v3, layout=(1, 3))
        push!(plts_traj, plt)
    end

    # Compute min/max values for each axis
    min_x = minimum([minimum(df.x1) for df in solution_dataframes[1:3]])
    max_x = maximum([maximum(df.x1) for df in solution_dataframes[1:3]])
    min_y = minimum([minimum(df.x2) for df in solution_dataframes[1:3]])
    max_y = maximum([maximum(df.x2) for df in solution_dataframes[1:3]])
    min_z = minimum([minimum(df.x3) for df in solution_dataframes[1:3]])
    max_z = maximum([maximum(df.x3) for df in solution_dataframes[1:3]])

    min_plot_x = min_x - 0.1 * (max_x - min_x)
    max_plot_x = max_x + 0.1 * (max_x - min_x)
    min_plot_y = min_y - 0.1 * (max_y - min_y)
    max_plot_y = max_y + 0.1 * (max_y - min_y)
    min_plot_z = min_z - 0.1 * (max_z - min_z)
    max_plot_z = max_z + 0.1 * (max_z - min_z)
    ...
    # Define edges as pairs of point indices
    edges = [
    ...
    # Plot
    for (i, j) in edges
        Plots.plot!(plt_extended_bounding_box, [corners[i][1], corners[j][1]],
    ...
    # Label axes and save
    Plots.plot!(plt_extended_bounding_box, xlabel="x", ylabel="y", zlabel="z", legend=false)
    Plots.xlims!(plt_extended_bounding_box, (min_plot_x, max_plot_x))
    Plots.ylims!(plt_extended_bounding_box, (min_plot_y, max_plot_y))
    Plots.zlims!(plt_extended_bounding_box, (min_plot_z, max_plot_z))
    Plots.savefig(plt_extended_bounding_box, "lorenz_training_data_structure_extended_bounding_box_err_" * string(error_level) * ".png")

    plt_data_structure = Plots.plot(plts_traj..., layout=(3, 1))
    Plots.savefig("lorenz_training_data_structure_err_" * string(error_level) * ".png")

    #save as png
    Plots.savefig("lorenz_training_data_structure_err_" * string(error_level) * ".png")

    serialize("lorenz_training_data_structure_err_" * string(error_level) * ".jld", data_structure)
end