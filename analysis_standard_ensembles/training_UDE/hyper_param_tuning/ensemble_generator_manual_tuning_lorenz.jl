...
function train(approximating_neural_network, training_dataframes, validation_dataframes, solution_dataframes, rng, learning_rate_adam, integrator, abstol, reltol, sensealg, seed)
  ...
  ude_parameters = vec(rand(rng, length(original_ude_parameters), 1))
    ...
    sols = [
      solve(
        remake(
          ...
          #u0=θ.u0[:, first(rg)]
          #u0=θ.u0[i, :, first(rg)]
          u0=original_training_data[i, :, first(rg)]
        ),
        integrator;
        saveat=tsteps[rg],
        reltol=reltol,
  ...
  function loss(θ, hyperparameters)
    ...
    loss_tmp =  loss_1[1] + loss_2[1] + loss_3[1]
    
    weights = vcat([vec(θ.p.p_net[layer_name].weight) for layer_name in keys(θ.p.p_net)]...)
    if regularization == 1
      loss_tmp = loss_tmp + regularization_coefficient_1 * sum(abs, weights)
    elseif regularization == 2
      loss_tmp = loss_tmp + regularization_coefficient_1 * sum(abs2, weights)
    elseif regularization == 3
      loss_tmp = loss_tmp + regularization_coefficient_1 * sum(abs2, weights) + regularization_coefficient_2 * sum(abs, weights)
    end

    return loss_tmp, hyperparameters
  end
  ...
  function callback2(θ, l, validation_losses, epochs)
    ...
    if epoch % 20 == 0
      ...
      optimized_sigma = θ.u.p.σ
      optimized_r = θ.u.p.r

      println("Optimized parameters: ", optimized_sigma, " ", optimized_r)
      ...
      plts = [Plots.scatter(solution_dataframes[1].t, [solution_dataframes[1].x1, solution_dataframes[1].x2, solution_dataframes[1].x3], label=["ground truth x1" "ground truth x2" "ground truth x3"]),
              Plots.scatter(solution_dataframes[2].t, [solution_dataframes[2].x1, solution_dataframes[2].x2, solution_dataframes[2].x3], label=["ground truth x1" "ground truth x2" "ground truth x3"]),
              Plots.scatter(solution_dataframes[3].t, [solution_dataframes[3].x1, solution_dataframes[3].x2, solution_dataframes[3].x3], label=["ground truth x1" "ground truth x2" "ground truth x3"]),
              ]

      Plots.plot!(plts[1], model_unique_prediction_1.t, Array(model_unique_prediction_1)', label=["pred x1" "pred x2" "pred x3"])
      title!(plts[1], "Total Epoch: $epoch")
      Plots.plot!(plts[1], legend=false)

      Plots.plot!(plts[2], model_unique_prediction_2.t, Array(model_unique_prediction_2)', label=["pred x1" "pred x2" "pred x3"])
      title!(plts[2], "Total Epoch: $epoch")
      Plots.plot!(plts[2], legend=false)

      Plots.plot!(plts[3], model_unique_prediction_3.t, Array(model_unique_prediction_3)', label=["pred x1" "pred x2" "pred x3"])
      ...
      rounded_reg_loss = round(regularization_loss, digits=6)
      println("Epoch: ", epoch, " Validation Loss: ", rounded_val_loss, " Loss: ", rounded_l, " Regularization Loss: ", rounded_reg_loss)
      flush(stdout)
    end
  ...
  function callback(θ, l, hyperparameters, validation_losses, epochs)
    ...
    if epoch % 20 == 0
      ...
      plts = [Plots.scatter(training_dataframes[1].t, [training_dataframes[1].x1, training_dataframes[1].x2, training_dataframes[1].x3], label=["ground truth x1" "ground truth x2" "ground truth x3"], alpha=.3),
              Plots.scatter(training_dataframes[2].t, [training_dataframes[2].x1, training_dataframes[2].x2, training_dataframes[2].x2], label=["ground truth x1" "ground truth x2" "ground truth x3"], alpha=.3),
              Plots.scatter(training_dataframes[3].t, [training_dataframes[3].x1, training_dataframes[3].x2, training_dataframes[3].x2], label=["ground truth x1" "ground truth x2" "ground truth x3"], alpha=.3),
              Plots.scatter(solution_dataframes[1].t, [solution_dataframes[1].x1, solution_dataframes[1].x2, solution_dataframes[1].x3], label=["ground truth x1" "ground truth x2" "ground truth x3"]),
              Plots.scatter(solution_dataframes[2].t, [solution_dataframes[2].x1, solution_dataframes[2].x2, solution_dataframes[2].x3], label=["ground truth x1" "ground truth x2" "ground truth x3"]),
              Plots.scatter(solution_dataframes[3].t, [solution_dataframes[3].x1, solution_dataframes[3].x2, solution_dataframes[3].x3], label=["ground truth x1" "ground truth x2" "ground truth x3"]),
              ]

      Plots.plot!(plts[4], model_unique_prediction_1.t, Array(model_unique_prediction_1)', label=["pred x1" "pred x2" "pred x3"])
      title!(plts[4], "Total Epoch: $epoch")
      Plots.plot!(plts[4], legend=false)

      Plots.plot!(plts[5], model_unique_prediction_2.t, Array(model_unique_prediction_2)', label=["pred x1" "pred x2" "pred x3"])
      title!(plts[5], "Total Epoch: $epoch")
      Plots.plot!(plts[5], legend=false)

      Plots.plot!(plts[6], model_unique_prediction_3.t, Array(model_unique_prediction_3)', label=["pred x1" "pred x2" "pred x3"])
      ...
      for count_rg in 1:length(model_predictions_1)
        ...
        Plots.scatter!(plts[1], [training_dataframes[1].t[first(rg)]], [[training_dataframes[1].x1[first(rg)]], [training_dataframes[1].x2[first(rg)]], [training_dataframes[1].x3[first(rg)]]], color="red")
        Plots.plot!(plts[1], legend=false)
      end
      ...
      for count_rg in 1:length(model_predictions_2)
        ...
        rg = hyperparameters.ranges[count_rg]
        Plots.scatter!(plts[2], [training_dataframes[2].t[first(rg)]], [[training_dataframes[2].x1[first(rg)]], [training_dataframes[2].x2[first(rg)]], [training_dataframes[2].x3[first(rg)]]], color="red")
        Plots.plot!(plts[2], legend=false)
      end
      ...
      for count_rg in 1:length(model_predictions_3)
        ...
        rg = hyperparameters.ranges[count_rg]
        Plots.scatter!(plts[3], [training_dataframes[3].t[first(rg)]], [[training_dataframes[3].x1[first(rg)]], [training_dataframes[3].x2[first(rg)]], [training_dataframes[3].x3[first(rg)]]], color="red")
        Plots.plot!(plts[3], legend=false)
      end
  ...
  original_training_data = deepcopy(u0)
  ...
  res = Optimization.solve(optprob2, Optim.LBFGS(), callback=(θ, l) -> callback2(θ, l, validation_losses, epochs), maxiters=1000)
