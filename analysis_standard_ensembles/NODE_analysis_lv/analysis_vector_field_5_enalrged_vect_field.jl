...
ensemble = deserialize("ensemble_results_model_1_gain_1.0_reg_0_0.0_to_keep.jld")
min_y1, max_y_1, min_y2, max_y2 = boundig_box_vect_field
original_min_y1, original_max_y_1, original_min_y2, original_max_y2 = boundig_box_vect_field
...
plt = Plots.scatter([p[1] for p in points_to_plot], [p[2] for p in points_to_plot], label="", xlabel="x", ylabel="y", zcolor=ground_truth_in_ensemble_to_plot, color=:viridis, markerstrokewidth=0, markersize=4, dpi = 1500)
...
# Draw red rectangle for original bounding box
rectangle_x = [original_min_y1, original_max_y_1, original_max_y_1, original_min_y1, original_min_y1]
rectangle_y = [original_min_y2, original_min_y2, original_max_y2, original_max_y2, original_min_y2]
Plots.plot!(plt, rectangle_x, rectangle_y, color=:red, linewidth=3, label="")


Plots.savefig(plt, result_image_folder * "/analysis_vector_field_ensemble_5_new.png")
...
plt = Plots.scatter([p[1] for p in points_to_plot], [p[2] for p in points_to_plot], label="sampled points", xlabel="y1", ylabel="y2", zcolor=magnitude_confidence_region, color=:viridis, markerstrokewidth=0, markersize=4, alpha = 0.5)
...
points = [(x, y) for x in range(min_y1, max_y_1, length=20), y in range(min_y2, max_y2, length=20)]
points = vec(points)
n_points = length(points)
