...
ground_truth_in_ensemble = reshape(ground_truth_in_ensemble, 100, 100)


x_coord = unique([p[1] for p in points])
y_coord = unique([p[2] for p in points])

blue_to_yellow = cgrad([RGB(0.02, 0.02, 0.1), :yellow])

contourf(x_coord, y_coord, ground_truth_in_ensemble, levels=10, color=blue_to_yellow)
...
Plots.plot!(plt, title="MCP: $mean_cicp", legend=:topleft)
Plots.plot!(plt, 
