...
experimental_data = deserialize("../data_generator_traj_1/lotka_volterra_in_silico_data_no_noise.jld")
...
mean_contained_for_dimensions = Dict()


for ensemble_dimension in ensemble_dimensions

    ensemble_1_tmp = ensemble_1[1:(ensemble_dimension*5)]
    ensemble_2_tmp = ensemble_2[1:(ensemble_dimension*5)]
    ensemble_3_tmp = ensemble_3[1:(ensemble_dimension*5)]

    tmp_ensemble = vcat(ensemble_1_tmp, ensemble_2_tmp, ensemble_3_tmp)

    ensembles = [tmp_ensemble[(k*ensemble_dimension+1):(k*ensemble_dimension+ensemble_dimension)] for k in 0:14]
    ...
    mean_contained_for_dimensions[ensemble_dimension] = mean_contained

end

# Create boxplots for the two sets of results
plt = Plots.plot()
for (ensemble_dimension, mean_contained) in mean_contained_for_dimensions
    Plots.boxplot!(plt, mean_contained, label=ensemble_dimension, legend=false, color=:lightblue)
end
Plots.plot!(plt, ylims=(0, 1), legend=false)
Plots.plot!(plt, xlabel="Ensemble size", ylabel="MPICP")
Plots.plot!(plt, xticks=(1:length(ensemble_dimensions), ensemble_dimensions))


Plots.savefig(plt, result_image_folder * "/trajectory_results_comparison_for_ensemble_dimension.png")

serialize(result_image_folder * "/cicps_distributions_trajectories.jld", mean_contained_for_dimensions)
