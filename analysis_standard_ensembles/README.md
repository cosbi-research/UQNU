...
This folder contains the scripts to run the proposed method in the partially data-driven case, with fixed values of mechanistic parameters -the models are Universal Differential Equations (UDEs), the parameters of the mechanistic component of the models are fixed to their ground truth values- for the three dynamical systems: **damped oscillator**, **Lorenz system**, and **Lotka-Volterra** with $\mathcal{L}_{\text{acc}} = 10^{-3}$. The structure includes configuration files, result analyses, launcher scripts, and utility modules.
