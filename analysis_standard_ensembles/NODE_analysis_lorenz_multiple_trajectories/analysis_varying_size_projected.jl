...
function get_analysis_ci(i, ensemble, points)
...
function analyze_method(index, total_ensemble, points)

    println("**********************Analyzing method*********************")

    ensemble = [member for member in total_ensemble[index]]
    results = [get_analysis_ci(i, ensemble, points) for i in 1:n_points]
...
for ensemble_dimension in ensemble_dimensions
    ...
    results_ensembles = [analyze_method(index, tmp_ensemble, points) for index in indexes]
    ...
    Plots.scatter!(plt, [p[1] for p in points_to_plot], [p[2] for p in points_to_plot], label="", xlabel="v1", ylabel="v2", title="", zcolor=ground_truth_in_ensemble_to_plot, color=:viridis, markerstrokewidth=0, markersize=4, dpi = 1500)
    Plots.scatter!(plt, [s[1] for s in experimental_points_centered_basis], [s[2] for s in experimental_points_centered_basis], label="", alpha=alphas, legend=false, color=:orange)
    #plot with the legend outside
    Plots.plot!(plt, title="Ensemble size $ensemble_dimension, mean CP: $mean_cicp", legend=:topleft)
    Plots.plot!(plt, xlims=(min_x, max_x), ylims=(min_y, max_y))
    Plots.plot!(plt, legend=:topleft)
    Plots.savefig(plt, result_image_folder * "/ground_truth_in_ensemble_" * string(ensemble_dimension) * ".png")
