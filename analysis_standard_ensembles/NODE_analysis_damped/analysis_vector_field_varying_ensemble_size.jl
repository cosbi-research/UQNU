...
for dimension_ensemble in dimensions_ensemble
    sampled_seeds = [seeds[(k*dimension_ensemble+1):(k*dimension_ensemble+dimension_ensemble)] for k in 0:4]

    results_ensembles = [analyze_method(sampled_seed, ensemble, 32, threshold) for sampled_seed in sampled_seeds]
