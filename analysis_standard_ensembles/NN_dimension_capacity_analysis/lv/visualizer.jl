...
ground_truth_vect_field = [lotka_volterra_ground_truth([p[1], p[2]]) for p in points]
...
#plot as a scatterplot with points colored by the squared error
...
function lotka_volterra_neural_node!(du, u, p, t)
...
for i in 1:length(initial_points) 
    ...
    prob = ODEProblem(lotka_volterra_neural_node!, starting_point, tspan, original_parameters)
...
for i in 1:length(simulations)
    ...
    squared_error = mean(abs2.(Array(sol) .- Array(sol_neural_ode)) ./ abs2.(Array(sol)))
...
for i in 1:length(initial_points)
    starting_point = initial_points[i]
    prob = ODEProblem(lotka_volterra!, starting_point, tspan, original_parameters)
    sol = solve(prob, Tsit5(), saveat=0.001, abstol=1e-6, reltol=1e-7)
...
plt = Plots.plot(simulations[1], vars=(1, 2), label="Ground truth", xlabel="x", ylabel="y", title="", dpi=600)
...
plt = Plots.plot(simulations[2], vars=(1, 2), label="Ground truth", xlabel="x", ylabel="y", title="", dpi=600)
...
plt = Plots.plot(simulations[3], vars=(1, 2), label="Ground truth", xlabel="x", ylabel="y", title="", dpi=600)
...
plt = Plots.plot(simulations[4], vars=(1, 2), label="Ground truth", xlabel="x", ylabel="y", title="", dpi=600)
...
plt = Plots.plot(simulations[5], vars=(1, 2), label="Ground truth", xlabel="x", ylabel="y", title="", dpi=600)
...
plt = Plots.plot(simulations[6], vars=(1, 2), label="Ground truth", xlabel="x", ylabel="y", title="", dpi=600)
