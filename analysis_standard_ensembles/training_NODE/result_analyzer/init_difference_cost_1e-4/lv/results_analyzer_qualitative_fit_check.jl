...
training_data_structure = deserialize("../../../data_generator/lv_training_ds_err_1.jld")
...
function compute_likelihood_on_trajectory(experimental_data_tmp, model_simulation, max_oscillations_tmp)
    ...
    n = size(experimental_data_tmp.t, 1)

    return 1/n * (sum(abs2.(residuals_first)) .+ sum(abs2.(residuals_second)))
end
...
function analyze_results(results, extended_name, validation_likelihood_threshold)
    ...
    Plots.savefig(plt, "lv_ensemble_"*extended_name*"_traj_1.png")
    ...
    plt = Plots.plot(plts[1], plts[2], layout=(2, 1), size=(1000, 500), xlabel="", ylabel="Population", legend=:bottomright)
    Plots.savefig(plt, "lv_ensemble_"*extended_name*"_traj_2.png")
    ...
    plt = Plots.plot(plts[1], plts[2], layout=(2, 1), size=(1000, 500), xlabel="", ylabel="Population", legend=:bottomright)
    Plots.savefig(plt, "lv_ensemble_"*extended_name*"_traj_3.png")
...
results_to_keep = likelihoods_1 .< 0.0001

results_1_to_keep = results_1[results_to_keep]

#save the results_to:_keep
serialize("ensemble_results_model_1_gain_1.0_reg_0_0.0_to_keep.jld", results_1_to_keep)
