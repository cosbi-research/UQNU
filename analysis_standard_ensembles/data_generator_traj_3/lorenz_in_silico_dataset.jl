...
serialize("bounding_box_trajectory_lorenz_3.jld", [min_y1_new, max_y1_new, min_y2_new, max_y2_new, min_y3_new, max_y3_new])
...
sol_1 = solve(prob, Tsit5(), u0=original_u0_1, saveat=0.001, abstol=abs_tol, reltol=rel_tol)
sol_2 = solve(prob, Tsit5(), u0=original_u0_2, saveat=0.001, abstol=abs_tol, reltol=rel_tol)
sol_3 = solve(prob, Tsit5(), u0=original_u0_3, saveat=0.001, abstol=abs_tol, reltol=rel_tol)
...
df_pure = DataFrame(t = times, x1 = sol_as_array_1[1,:], x2 = sol_as_array_1[2,:], x3 = sol_as_array_1[3,:], traj = repeat([1], length(times)))
df_pure = vcat(df, DataFrame(t = times, x1 = sol_as_array_2[1,:], x2 = sol_as_array_2[2,:], x3 = sol_as_array_2[3,:], traj = repeat([2], length(times))))
df_pure = vcat(df, DataFrame(t = times, x1 = sol_as_array_3[1,:], x2 = sol_as_array_3[2,:], x3 = sol_as_array_3[3,:], traj = repeat([3], length(times))))
#save the data
serialize("lorenz_in_silico_data_no_noise_pure_trajectory.jld", df_pure)
