...
#save the plot
Plots.savefig(plt, "experimental_data_in_new_basis.svg")
...
enlarged_min_y1 = original_min_y1 - 0.2 * (original_max_y1 - original_min_y1)
enlarged_max_y1 = original_max_y1 + 0.2 * (original_max_y1 - original_min_y1)
enlarged_min_y2 = original_min_y2 - 0.2 * (original_max_y2 - original_min_y2)
enlarged_max_y2 = original_max_y2 + 0.2 * (original_max_y2 - original_min_y2)
enlarged_min_y3 = original_min_y3 - 0.2 * (original_max_y3 - original_min_y3)
enlarged_max_y3 = original_max_y3 + 0.2 * (original_max_y3 - original_min_y3)

# enlarge the bounding box of the 30%
original_min_y1 = enlarged_min_y1  
original_max_y1 = enlarged_max_y1
original_min_y2 = enlarged_min_y2
original_max_y2 = enlarged_max_y2
original_min_y3 = enlarged_min_y3
original_max_y3 = enlarged_max_y3
    ...
    get_uode_model_function = function (appr_neural_network, state)
        ...
        f(u, p) =
            let appr_neural_network = appr_neural_network, st = state
                ...
                σ= 10.0
                r = 28.0
                du[1] = σ*(u_reprojected[2] - u_reprojected[1]) 
                du[2] = u_reprojected[1]*(r-u_reprojected[3])- u_reprojected[2]
                du[3] = û[1]
                return du
            end
    end
...
mean_cicps = deserialize("cicps_distribution_naive_extended.jld")
mean_cicp = mean(mean_cicps)

gr()
plt = Plots.plot()
Plots.scatter!(plt, [s[1] for s in experimental_points_centered_basis], [s[2] for s in experimental_points_centered_basis], label="training points", color=:orange, dpi=600)
...
Plots.savefig(plt, "ground_truth_in_ensemble_projected_standard_extended.svg")
Plots.savefig(plt, "ground_truth_in_ensemble_projected_standard_extended.png")
