...
not_failed_results = filter(res -> res.status == "success", results)
...
for ensemble_index in 1:10
  ...
  initial_index = (ensemble_index - 1) * 5 + 1
  end_index = min(initial_index + 5 - 1, length(not_failed_results))

  for res in not_failed_results[initial_index:end_index]
    ...
    θ = res.training_res
    θ.u0[1, 4, 1] = 2.9 * 10^3 / 10^5

    println("Visualizing survival result with u0: ", θ.u0[1, 4, 1])
  ...
  #circle the forth variable which is the observable
  savefig(p, "cell_apop_UDE_results/v_se_$ensemble_index.png")
  ...
  for var_index in 1:size(training_datas[1], 1)
    ...
    println("Variable $(var_index) inside CI: ", inside_ci)
  ...
  for t_index in 1:length(contained_points)
    contained = 1
    for var_index in 1:length(variable_contained_in_ci)
  ...
  push!(trajectory_contained, contained_points)

  #plot in a grid layout
  p = Plots.plot(variable_plots..., layout=(4, 2), size=(900, 900))
  #circle the forth variable which is the observable
  savefig(p, "cell_apop_UDE_results/v_se_ci_ensemble_$ensemble_index.png")
