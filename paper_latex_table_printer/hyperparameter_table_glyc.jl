#= 
Script to print the table of tuned hyperparameters for the yeast glycolysis UDE model trained on DS_00 and DS_05
=#
...
########## error 0.0 ################
hyperparameters_00 = deserialize("results_hyperparameter_tuned/glyc_00.jld")
hyperparameters_00_best_trial = deserialize("results_hyperparameter_tuned/hyperparameters_glyc_00_best_trial.jld")

########## error 0.05 ###############
hyperparameters_05 = deserialize("results_hyperparameter_tuned/glyc_05.jld")
hyperparameters_05_best_trial = deserialize("results_hyperparameter_tuned/hyperparameters_glyc_05_best_trial.jld")
...
template_row = read("latex_table_templates/template_hyper_parameters_stiff.txt", String)
...
for dict_key in keys(hyperparameters_00)
    if startswith(String(dict_key), "p")
        ...
        push!(parameter_names_wrong_order, pos)
        ...
        hyperparameters_00_to_insert = hyperparameters_00[dict_key]
        hyperparameters_05_to_insert = hyperparameters_05[dict_key]

        if pos == 3
            hyperparameters_00_to_insert = 10 ^ hyperparameters_00_to_insert
            hyperparameters_05_to_insert = 10 ^ hyperparameters_05_to_insert
        end

        parameter_row = replace(parameter_row, "##value_00##" => get_formatted_double(hyperparameters_00_to_insert))
        parameter_row = replace(parameter_row, "##value_05##" => get_formatted_double(hyperparameters_05_to_insert))

        push!(parameter_rows, parameter_row)
    end
end

parameter_rows = parameter_rows[sortperm(parameter_names_wrong_order)]
