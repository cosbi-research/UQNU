...
function compute_confidence_ellipse(data::Matrix, confidence_level::Float64=0.95)
    ...
    eig_vals, eig_vecs = eigen(Symmetric(cov_matrix))
    ...
    axes_lengths = sqrt.(abs.(eig_vals * statistics_value))
...
function is_point_inside_ellipse(point::Vector, ellipse::NamedTuple)
    ...
    semiaxis_lengths = max.(ellipse.semiaxis_lengths, 1e-12)  # Avoid division by zero
    # Step 3: Check if the point is inside the ellipse
    return sum((projections ./ semiaxis_lengths) .^ 2) <= 1
end
