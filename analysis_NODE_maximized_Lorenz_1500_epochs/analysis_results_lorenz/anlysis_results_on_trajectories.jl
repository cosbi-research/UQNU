...
plt = Plots.boxplot(["CP MOD 800 epochs" for _ in naive_trajectory_results], naive_trajectory_results, label="CP MOD 800 epochs", alpha=0.5, color=:lightblue, xlabel="", ylabel="CP")
Plots.boxplot!(plt, ["CP MOD 1500 epochs" for _ in maximized_trajectory_results], maximized_trajectory_results, label="CP 1500 epochs", alpha=0.5, color=:lightblue)
...
for trajectory_index in axes(maximized_trajectory_results[3], 1)
    ...
    plt1 = Plots.plot(dpi=600)
    plt2 = Plots.plot(dpi=600)
    plt3 = Plots.plot(dpi=600)
