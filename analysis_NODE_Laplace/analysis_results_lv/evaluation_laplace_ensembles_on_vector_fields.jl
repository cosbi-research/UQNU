...
naive_ensembles = []
for i in 1:10
    maximized_ensemble_folder = "../results_Laplace/lv/result_lv_laplace$i/ensemble_laplace_approximation$i.jld"
    if !isfile(maximized_ensemble_folder)
        continue
    end
    tmp_results = deserialize(maximized_ensemble_folder)
    tmp_ensemble = tmp_results[1:100]
...
serialize("results_analysis_laplace.jld", results)

results_deserialized = deserialize("results_analysis_laplace.jld")
...
Plots.plot!(plt, title="Lotka-Volterra (Laplace)", legend=:topleft)
Plots.plot!(plt, 
...
Plots.savefig(plt, "ground_truth_in_ensemble_laplace.png")
Plots.savefig(plt, "ground_truth_in_ensemble_laplace.svg")
...
serialize("cicps_distribution_laplcae.jld", single_cicps)
...
plt = Plots.boxplot(["Laplace" for _ in single_cicps], single_cicps, label="Standard", xlabel="", ylabel="CP", alpha=0.5, color=:lightblue)
Plots.boxplot!(plt, ["Maximized OOD disagreement" for _ in single_cicps_maximized], single_cicps_maximized, label="MOD", alpha=0.5, color=:lightblue)
