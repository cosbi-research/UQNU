...
function getNextPointDirection(par, times, initial_states, training_data_structure, out_of_domain_points, current_ensemble, ood_analyzer, parameter_index, step_size, trajectories, iterator)
  ...
  if length(current_ensemble) < 3 || iterator < 200
...
number_iterations_for_trajectory = 1600
validation_cost_threshold = 1e-4
...
sampled_indexes_trajectories = [sort(sample(total_parameter_indexes, 300; replace=false)) for i in 1:number_of_trajectories]
      ...
      while iteration_performed < 200
        ...
        for iterator in 1:number_iterations_for_trajectory
          ...
          if iterator == 1
            ...
            while validation_cost < validation_cost_threshold && (validation_cost - previous_validation_cost) < 0.0000001
