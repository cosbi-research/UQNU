...
- **`training_NODE_results/`**  
  This folder contains the results of standard ensembling on the three different dynamical systems. One random model from each standard ensemble is used as the starting point for our method. The threshold to accept a model in the standard ensembles is $\mathcal{L}_{\text{acc}} = 10^{-4}$.
...
- `routing_loss_contour_lv.jl`  
  Main scripts that implement the method described in the paper, with $\mathcal{L}_{\text{acc}} = 10^{-4}$.
