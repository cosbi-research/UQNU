#= 
Script to run the training of the cell apoptosis UDE model on the dataset with error level 0.0, fixing p2 and p4 to their literature values and
...
function train_uode_model(seed, iterator)
  ...
  function callback(θ, l, training_epochs, training_costs, best_training_parameters) #callback function to observe training
  ...
  try
    ...
    res = Optimization.solve(optprob2, Optim.LBFGS(), callback=(θ, l) -> callback(θ, l, training_epochs, training_costs, best_training_parameters), maxiters=5, allow_f_increases=true)
  catch
    println("error in BFGS")
  end
