#= 
Script to run the hyperparameter optimization for the Yeast Glycolysis model using the TPE algorithm with error level 5.0%
=#
...
function objective(original_ode_par, learning_rate_adam, num_hidden_layers, num_hidden_nodes, ms_group_size, ms_continuity_term, seed)
  ...
  function validation_loss_function(θ)
    ...
    try
      ...
      loss = 1 / size(validation_df, 1) * sum(abs2.(Array(validation_df[:, 2:end])' .- x) ./ abs2.(Array(validation_df_sd[:, 2:end])'))
