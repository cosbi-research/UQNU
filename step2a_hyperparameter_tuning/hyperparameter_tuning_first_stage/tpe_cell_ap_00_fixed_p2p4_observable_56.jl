#= 
Script to run the hyperparameter optimization for the cell apoptosis model using the TPE algorithm with error level 0.0%,
fixing the parameters p2 and p4 to their literature values, assuming that only y5 and y6 are observable.
=#
...
result_name_string = "cell_ap_00.jld"
...
#generates the latin hypercube sampling for the initial parameter real_values
...
function objective(trial)
  ...
  original_p3 = trial.suggest_float("p3", lower_bounds[2], upper_bounds[2])
  original_p5 = trial.suggest_float("p5", lower_bounds[3], upper_bounds[3])
  original_p6 = trial.suggest_float("p6", lower_bounds[4], upper_bounds[4])
  original_p7 = trial.suggest_float("p7", lower_bounds[5], upper_bounds[5])
  original_p8 = trial.suggest_float("p8", lower_bounds[6], upper_bounds[6])
  original_p9 = trial.suggest_float("p9", lower_bounds[7], upper_bounds[7])
  ...
  original_parameter_vector = [original_p1, original_p3, original_p5, original_p6, original_p7, original_p8, original_p9]

  #gets the hybrid derivative function and instatiate the prediction ODE problem
  uode_derivative_function = get_uode_model_function_fixed_p2p4(approximating_neural_network, st, original_parameter_vector)
  # mechanistic parmeter scaled to the initial value
  initial_parameters = ones(7)
  ...
  function loss_function(data, deviation, pred)
    original_cost = sum(abs2.(data[observables, :] .- pred[observables, :]) ./ deviation[observables, :] .^ 2)
  ...
  try
    ...
    result_params["p3"] = min(max(ode_par_optimized[2], lower_bounds[2]),upper_bounds[2])
    result_params["p5"] = min(max(ode_par_optimized[3], lower_bounds[3]),upper_bounds[3])
    result_params["p6"] = min(max(ode_par_optimized[4], lower_bounds[4]),upper_bounds[4])
    result_params["p7"] = min(max(ode_par_optimized[5], lower_bounds[5]),upper_bounds[5])
    result_params["p8"] = min(max(ode_par_optimized[6], lower_bounds[6]),upper_bounds[6])
    result_params["p9"] = min(max(ode_par_optimized[7], lower_bounds[7]),upper_bounds[7])
