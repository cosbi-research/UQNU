#= 
Script to run the hyperparameter optimization for the Yeast Glycolysis model using the TPE algorithm with error level 5%,
...
function objective(original_ode_par, learning_rate_adam, num_hidden_layers, num_hidden_nodes, ms_group_size, ms_continuity_term, seed)
  ...
  #computes the validation loss function
  function validation_loss_function(θ)

    loss = 0.0
    validation_df = validation_solution_dataframe
    validation_df_sd = validation_solution_sd_dataframe
    ...
    try
      ...
      loss = 1 / size(validation_df, 1) * sum(abs2.((Array(validation_df[:, 2:end])')[observables,:] .- x[observables,:]) ./ abs2.((Array(validation_df_sd[:, 2:end])')[observables,:]))
