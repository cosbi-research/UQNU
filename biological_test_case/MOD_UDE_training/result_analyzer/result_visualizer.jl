...
for ensemble_index in 1:10
  ...
  for index in axes(naive_ensemble, 1)
    ...
    if length(variable_plots) == 0
      for var_index in 1:size(training_datas[1], 1)
        ...
        p = Plots.plot()
        plot!(p, solutions_dataframe.t, sol[var_index, :], label="", lw=1, color=:red)
        #range 0-1 for y axis
        y_min = minimum(solutions_dataframe[:, var_index+1])
        y_max = maximum(solutions_dataframe[:, var_index+1])
        y_range = y_max - y_min
  ...
  for res in MOD_ensemble
    ...
    #plot the results towards the training data 
    if length(variable_plots_MODs) == 0
      for var_index in 1:size(training_datas[1], 1)
        ...
        y_upper = y_max + 0.1 * y_range
        Plots.plot!(p, ylims=(y_lower, y_upper))
        push!(variable_plots_MODs, p)
      end
    else
      for var_index in 1:size(training_datas[1], 1)
        p = variable_plots_MODs[var_index]
        plot!(p, solutions_dataframe.t, sol[var_index, :], label="", lw=1, color=:red)
        y_min = minimum(solutions_dataframe[:, var_index+1])
        y_max = maximum(solutions_dataframe[:, var_index+1])
        y_range = y_max - y_min
  ...
  #plot in a grid layout
  p = Plots.plot(variable_plots_MODs..., layout=(4, 2), size=(900, 900))
  #circle the forth variable which is the observable
  ...
  aleatoric_uncertainty = zeros(size(naive_ensemble, 1))
  for ensemble_index in 1:size(naive_ensemble, 1)
    pred = predictions[ensemble_index][observables, :]
    tmp_data = training_datas[1][observables, :]

    aleatoric_uncertainty[ensemble_index] = 1 / size(tmp_data, 2) * sum(abs2.(tmp_data .- pred))
  end

  MOD_aleatoric_uncertainty = zeros(size(MOD_ensemble, 1))
  for ensemble_index in 1:size(MOD_ensemble, 1)
    pred = MOD_predictions[ensemble_index][observables, :]
    tmp_data = training_datas[1][observables, :]

    MOD_aleatoric_uncertainty[ensemble_index] = 1 / size(tmp_data, 2) * sum(abs2.(tmp_data .- pred))
  end

  sigma_naive_ensembles = sqrt.(aleatoric_uncertainty)
  sigma_mod_ensembles = sqrt.(MOD_aleatoric_uncertainty)
  ...
  for t_index in 1:size(predictions[1], 2)
    ...
    std_prediction[observables, t_index] .= sqrt(mean(aleatoric_uncertainty .+ vec(hcat(preds_at_t...)[observables, :] .^ 2)) - mean_prediction[observables, t_index][1]^2)
    non_observables = setdiff(axes(std_prediction, 1), observables)
    std_prediction[non_observables, t_index] .= 0
    ...
    MOD_std_prediction[observables, t_index] .= sqrt(mean(MOD_aleatoric_uncertainty .+ vec(hcat(MOD_preds_at_t...)[observables, :] .^ 2)) - MOD_mean_prediction[observables, t_index][1]^2)
    MOD_std_prediction[non_observables, t_index] .= 0
  end
...
#plot in a grid layout
p = Plots.plot(variable_plots_MODs[4])
Plots.plot!(p,
  xlabel="Time (hours)",
  ylabel="y4 (10⁵ molecules/cell)"
)
...
for ensemble_index in 1:10
  ...
  for t_index in 1:size(predictions[1], 2)
    ...
    std_prediction[observables, t_index] .= sqrt(mean(sigma_naive_ensembles.^2 .+ vec(hcat(preds_at_t...)[observables, :] .^ 2)) - mean_prediction[observables, t_index][1]^2)
    non_observables = setdiff(axes(std_prediction, 1), observables)
    std_prediction[non_observables, t_index] .= 0
    ...
    std_prediction_MOD[observables, t_index] .= sqrt(mean(sigma_mod_ensembles .^ 2 .+ hcat(preds_at_t_MOD...)[observables, :] .^ 2) - mean_prediction_MOD[observables, t_index][1]^2)
    non_observables = setdiff(axes(std_prediction_MOD, 1), observables)
    std_prediction_MOD[non_observables, t_index] .= 0
  end
  ...
  for var_index in 1:size(training_datas[1], 1)
    ...
    #fill between upper and lower bound
    Plots.plot!(p, solutions_dataframe.t, upper_bound, fillrange=lower_bound, fillalpha=0.2, fillcolor=:orange, label="0.95-PI Standard", lw=0)
    Plots.plot!(p, solutions_dataframe.t, upper_bound_MOD, fillrange=lower_bound_MOD, fillalpha=0.2, fillcolor=:green, label="0.95-PI MOD", lw=0)
  ...
  #plot in a grid layout
  #circle the forth variable which is the observable
  savefig(variable_plots[4], "cell_apop_UDE_results/v_se_ci_ensemble_survival_$ensemble_index.png")

end
#trying and simulate the model on the other initial conditions (cell death)
#save the contained in ci 

#get mean and std of survival_trajectory_contained
mean_survival_contained = mean(survival_trajectory_contained)
std_survival_contained = std(survival_trajectory_contained)
sem_survival_contained = std_survival_contained / sqrt(length(survival_trajectory_contained))

mean_MOD_survival_contained = mean(MOD_survival_trajectory_contained)
std_MOD_survival_contained = std(MOD_survival_trajectory_contained)
sem_MOD_survival_contained = std_MOD_survival_contained / sqrt(length(MOD_survival_trajectory_contained))
